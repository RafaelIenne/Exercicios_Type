// Função que converte o preço em dólares para reais, incluindo o IOF e calcula o preço de venda | Lucro é colocado ja na função, para numero = a % dele (EX: lucro = 20, 20%)
function converterPrecoParaReais1(precoEmDolar1: number, taxaDeCambio1: number, iof1: number = 6.38, lucro1: number = 49): { precoEmReais1: number, valorIOF1: number, precoComIOF1: number, precoVenda1: number } {
    ...
    // Calcula o preço de venda com 49% de lucro
    const precoVenda1 = precoComIOF1 * (1 + lucro1 / 100); //precoVenda já pega o valor completo, portanto é o último a ser escrito
...
produtos1.forEach(produto1 => {
    ...
    console.log(`Preço de venda (com 49% de lucro): R$ ${precoVenda1.toFixed(2)}\n`); //Mostrado por ultimo para uma conclusão
});